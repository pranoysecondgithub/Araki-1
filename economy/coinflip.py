...
class CoinFlip(commands.Cog):
  ...
  async def Flip(self, ctx, amount = "1", ht = 'heads'):
    ...
    if amount == None:
      await ctx.reply("Please give a amount")
    elif coinflip_check is None:
      await ctx.message.reply("You don't have a profile!\nPlease execute the `create` command to create a profile!")
    else:
      ...
      if balance <= 0:
        ...
        message = await ctx.send(embed=embflip)
        await asyncio.sleep(10)
        emb3edit = nextcord.Embed(title=f"U had your luck !!", description=f"<:CoinBlue:948794998952980500> The coin spins and you won {amount}", colour=clr)
